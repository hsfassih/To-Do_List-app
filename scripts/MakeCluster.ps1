...
.DESCRIPTION
    ...
    3. Tags and pushes image to the k3d local registry (k3d-todo-registry:5000)
    4. Applies all manifests in k8s/
    5. Waits for pods to be Ready and prints the running resources

.NOTES
    The application will be accessible at: http://localhost:8080
    The local registry is available at: k3d-todo-registry:5000
...
$clusterExists = (k3d cluster list 2>$null | Select-String -Pattern "^$ClusterName")
if ($clusterExists) {
